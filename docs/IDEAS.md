...
| **Stack-pinning: record the Wine + winetricks version, refuse to merge across incompatible stacks.** WineHQ AppDB's hard rule — a test report is worthless unless you know exactly what stack produced it — and the fix ProtonDB applied late (its PC-vs-Deck split). | **DONE** — v4.9. Fields 9-11 appended; the resolver weighs a foreign stack at half. |
| **Recency / version decay in the resolver.** ProtonDB's documented weakness is that a stale report for an old Proton never decays; the fix is to weight a newer contradicting report over an old confirmation, and treat "confirmed only on Wine older than the caller's" as reduced confidence. | **DONE** — v4.9/4.4. `peso()` in `t_lista_linha`, with a floor of 0.25. |
| **Downgrade, don't overwrite, when an established fingerprint's verb set suddenly changes.** The AUR orphan-adoption lesson at the record level: a sudden verb-set flip on a fingerprint with history should *lower* confidence pending re-confirmation, not silently replace the lesson — that is a reputation-inheritance vector. | **DONE** — falls out of the decay arithmetic: a sudden verb-set flip loses on weight until enough machines report it. |
...
| Idea | Verdict |
|---|---|
| **"Por que este verbo?" — an explain-line before the pre-flight install.** Tandem already maps the missing DLL to a verb; say it in one sentence before installing ("this program is asking for MSVCP140.dll, which comes from the Visual C++ 2015–2022 runtime — I'll install that"). Turns a silent install into a legible one. It explains more and executes exactly the same, which is the criterion at the top of this file. | **DONE** — `t_verbo_amigavel` names the component in plain words before installing it. |
| **A repeatable field-test harness for a real shop program.** Not the weekly PuTTY/Notepad++ run — a `tests/real-shop.sh` the owner runs *on the counter machine* against the actual POS/accounting/fiscal installer, capturing every step of the closed loop to a report Tandem reads back. Turns "the one afternoon" into an instrument, and produces the first honest community-list record. | **WRITTEN, waiting for the counter** — `tests/real-shop.sh`, 4.16. The harness itself needed no shop: what needs the machine is RUNNING it. It aborts rather than asks if the prefix it would use lacks the `.tandem-prefixo` marker, and it answers the component question automatically while leaving "did it actually work" to a person - that field is the only evidence the list carries that a harness cannot manufacture. Writing it found four invented function names, a `t_dll_para_verbo` undefined at run time because common.sh does not source winedeps.sh, a `read` that could not tell "nobody was there" from "he could not tell", and a 25-second timeout that would have killed dotnet48 at the counter. **What remains is one afternoon with a real installer, and only the owner has one.** |
...
- **DONE in v4.53 — the PC/SC row was the best-conditioned item in the whole set.** Measured here: `winscard.so` is genuinely `NEEDED`-linked to `libpcsclite.so.1`; `70-uaccess.rules:51` is an unconditional `ENV{ID_SMARTCARD_READER}=="?*", TAG+="uaccess"`, so a real CCID reader is handed to the logged-in user with zero udev work; and `pcscd`, `libccid` and `pcsc-tools` all answer `Installed: (none)` on this stock image. That absence is usually the whole answer, and `limites.tsv` row 39 used to hand the owner a four-step procedure with no way to find out which step he is stuck on. Now `t_chave_estado pcsc` reads INSTALADO (is the pcsc software present) and SERVICO (is pcscd running), both unprivileged, and `t_texto_chave pcsc` appends one line to the verdict: "the smartcard service is NOT installed; that is almost always the whole problem" when absent, "does not appear to be running" when installed-but-stopped, and nothing extra when it is up — sitting beside the sentinel/codemeter families rather than being a special case (`pcsc_falta`, `pcsc_parado`, all seven languages). The CUPS half of item 4 stays open.
...
**Yes, and that is the defect — now two-thirds closed.** Four rows handed the owner a procedure and never looked to see which step he is stuck on: `winscard.dll` (install three packages, run `pcsc-scan`), `hid.dll` (it is almost always read permission), `rockey*.dll` (same guess), and the Sentinel family (a technician installs it, once). Every one of those is checkable by reading, unprivileged, before the download. The project already decided this shape is right when it built `apt-get install -s` into `tandem-deb` — nobody types a password to be told no — and then did not apply it to the one table whose whole job is telling people what to try. A row that describes a route without checking it is prose, and constraint 5 says prose is not a bridge. **The Sentinel family was closed in v4.52** (the installed-Wine-is-older honesty line), and **the `winscard.dll`/PC/SC row in v4.53** (the pcscd installed/running check — see item 4 above). What remains open here is `hid.dll` and `rockey*.dll` — both "almost always a read-permission problem", checkable by reading the device node's mode against the owner's groups the same way `t_no_grupo` already does for the lp/dialout warnings.
