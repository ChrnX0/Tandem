...
[![tests](https://img.shields.io/badge/tests-1918-brightgreen)](tests/run.sh)
...
**On Arch?** A `PKGBUILD` is attached to every release (alongside `tandem.install`), generated from the same layout and version as the `.deb`, so `pacman` tracks Tandem like any native package:

```bash
makepkg -si          # from a directory holding PKGBUILD and tandem.install
```

Once it is on the AUR, `yay -S tandem` (or your AUR helper) is all it takes.
...
| `tandem servico <folder>` | run a web service (Node, Python, PHP, Java, a binary, or an `.exe` under Wine) and keep it alive |
| `tandem relogio` | check the clock — a wrong one silently breaks TLS, licences and fiscal software (a dead CMOS battery is the usual cause) |
...
| `tandem historico` | a visual history of the programs it has handled |
